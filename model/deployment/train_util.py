...
def _load_weights():
    ...
    assert len(networks) == FLAGS.num_clones or len(networks) == 2 * FLAGS.num_clones
    network1 = networks[0]
    if len(networks) == 2 * FLAGS.num_clones:
        network2 = networks[1]
    ops = []
    for key in data_dict:
        try:
            if key in network1.layers.keys():
            ...
            if len(networks) == 2 * FLAGS.num_clones:
                if key in network2.layers.keys():
                    print('Restoring layer copy/%s'%key)
                    with tf.variable_scope("copy/%s"%key, reuse=True):
                        for subkey, data in data_dict[key].iteritems():
                            var = tf.get_variable(subkey)
                            if (len(data.shape) == 4 and
                                data.shape[2] == 3 and
